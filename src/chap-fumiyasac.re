...
その様な中、丁度良きタイミングで技書博11（2025年1月25日開催）の次回案内を制作をする運びになったので、以前から気になっていた画像生成AI「Adobe Firefly@<fn>{firefly}」をデザインの中で活用する事にしました。

本稿では、技書博11の次回案内デザインの過程において、どの様な形で画像生成AIを活用したかを簡単ではありますがご紹介できればと考えております。

//footnote[firefly][https://www.adobe.com/jp/products/firefly.html]
...
//image[001_firefly_control_panel][Adobe Fireflyの操作画面][scale=0.8]
...
※ 料金やクレジットに関する詳細は、生成AIを含むプランを比較@<fn>{plan}のWebページに詳細が掲載されています（ちなみに、私は「Adobe Creative Cloud」の年間サブスクリプションを契約して利用しています）。

//footnote[plan][https://www.adobe.com/jp/products/firefly/plans.html]
...
//image[002_sozai_sentei_no1][写真素材候補（書籍に囲まれている）][scale=0.8]
...
//image[003_sozai_sentei_no2][写真素材候補（横浜の豪華客船と夜景）][scale=0.8]
...
//image[appendix_creative_archive][これまで作成に携わった次回案内・参加募集のクリエイティブ][scale=0.8]
...
//image[004_making_photoshop_background][背景素材をPhotoshopで合成する][scale=0.8]
...
//image[005_making_photoshop_keyword][書籍からキーワードが湧き上がる部分の表現][scale=0.8]

※ 関連するキーワードの表現については、文字に対して枠線を作り、その枠線に対してぼかしを加える事で表現しています（制作当初はネオン光彩の様なデザインを考えていましたが、大きな明朝体でないと映えない事や加工手順が少し面倒に感じたので、今回は利用しませんでした。※参考:光る文字を作る@<fn>{neon}）。

//footnote[neon][https://www.mdn.co.jp/reference/Illustrator/170]
