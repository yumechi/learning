...
生成AIとしてマーケティングされているアプリケーション・サービスで、学びに活用出来そうなものを紹介していこう。なお、筆者はLLMが専門なため、画像生成や、音声、動画、音楽などといったサービスに関しては特に触れない。
...
他にも定番の使い方がある。あなたが生成AIに興味があるなら論文を読ませればいい。今時はarxivに様々な論文@<fn>{erukiti-preprint}が大量に公開されている。少なくとも生成AIの最先端で外部公開されている論文ならまず大半がarxivにある。
...
つまり、聞きたい内容はこちらが明示的に指示しなければならない。論文を読む場合、落合陽一先生の論文要旨のテンプレートがあるらしい。201x年代に他の人が公開しているスライドがslideshareにある@<fn>{erukiti-ochiai}ので、これを参考にしてみよう。
...
筆者はしがない専門学校卒なため、論文には疎いのだが対話型AIを活用すれば、自分に合った説明をいくらでも引き出せる。いい時代になったものだ。

なお、質問すべきことが思いつかない場合は、「あまり理解できてなくて質問が思いつかないんだけど、いい質問例はある?10個上げてみて」みたいなのを投げてみるといいだろう。
...
何かのデータを渡して読み込むのもありだが、他にも学びに特化した使い方はある。問題を作ってもらうというものだ。

「TypeScriptのconditional typeについて理解を深めたい。何か問題を出してほしい」と投げてみた。
...
もちろんここで「ヒントをください」と言えばヒントを教えてくれる。
...
ここで注意しないといけないことがある。問題を作ってもらう場合、ハルシネーションと呼ばれる嘘に注意しなければいけない。対話型AIの知識を無条件で信じ込むのは危険だ。TypeScriptのように世の中に情報が豊富にある技術なら、嘘をつかれにくいが、それでも嘘をつかれる可能性はある。特に解答の判定に関しては間違いが含まれることはある。

そのため何かしらかの方法で検証をするのが望ましい。TypeScriptなら、TypeScript Playground@<fn>{erukiti-ts-playground}で書いてみるとかだ。
...
コーディング支援AIは、ソフトウェア開発においてものすごく有用なものである。筆者が強くおすすめするGitHub Copilot@<fn>{erukiti-github-copilot}は、対応しているエディタ上でソースコードを入力すると自動でソースコードの続きが補完できる機能や、エディタ上のチャット窓でソースコードに関する相談を投げるとそのまま返答してくれる機能を持つ。

//footnote[erukiti-github-copilot][https://github.com/features/copilot]

他にもGitHubのコードレビューをしてくれるCodeRabbit@<fn>{erukiti-coderabbit}などがある。

//footnote[erukiti-coderabbit][https://www.coderabbit.ai/]

「学び」とは関連性が低いため、これらの詳細は割愛しよう。

== AI検索

AI検索には、MicrosoftのBingやChatGPTのsearch GPTや、perpelxity.ai@<fn>{erukiti-perplexity}などがある。筆者のおすすめはperplexityだ。perplexityは老舗中の老舗であり、検索AIにおいては他の追従を許さない。

//footnote[erukiti-perplexity][https://www.perplexity.ai/]

AI検索は、ユーザーのクエリを自然言語として解釈して、検索エンジンのクエリに変換して得た結果を元に回答するものだ。旧来の検索エンジンとは異なり、ユーザーの質問の意図や文脈を読み取ってくれる。

対話型AIは、知識を求めるのには向いていないが、AI検索であれば検索エンジンにヒットした情報には忠実に答えてくれる。そのためLLMが学習していない知識に関しても、ウェブ検索をした上で回答してくれるのだ。

ただし検索エンジンの弱点をそのまま引きずっている。SEOの強いものウェブページを参照しがちなため、検索汚染がひどいジャンルの場合、なかなか適切なドキュメントを参照してくれないという問題がある。perpelxityは長い歴史によるノウハウが貯まっているためかそれが大分マシである。search GPTやBingはその点において、とてもイライラさせられがちだ。

また、一度LLMで自然言語処理を行うため、どうしても応答速度は遅い。自分にとって自明なものを探し出す目的なら旧来型の検索エンジンの方がいいだろう。概要をつかみたい場合や、うろ覚えのものを調べたい場合などには、一定の効果がある。
