...
生成AIとは「生成するAI」である。馬鹿馬鹿しい定義だが、生成AIは研究・開発における専門用語でもなんでもなくて、単なるマーケティング用語だ。生成AIはその程度の情報量しかない薄っぺらい言葉にすぎないが、使い勝手の良い言葉でもある@<fn>{erukiti-genai}。
//footnote[erukiti-genai][なんやかんやで筆者も使ってしまう程度には使い勝手がいい。]

筆者はAIエンジニアである。主に大規模言語モデル（以後、Large Language Modelの略でLLMと書く）を取り扱っている。LLMは文字列を入れたら、それに応じた最もそれらしい文字列を返してくれる魔法の技術くらいに考えればいい。

@<chapref>{chap-erukiti1}で述べた通り、学びには「言語化」という重要な要素がある。じつはLLMは、我々の言語化のプロセスを強力に支援してくれる存在だ。漠然とした理解を明確な言葉に変換する手助けをしてくれるのだ。
...
OpenAI社（オープンAI）のChatGPT（チャットGPT）は対話型AIと呼ぶべきサービスである。同様のものにはAnthropic社（アンスロピック）のClaude（クロード）やGoogle社のGemini（ジェミニ）がある。

対話型AIは、対話、チャットによって、LLMに返答してもらうアプリケーションだ。利用者は何かしらの指示を出す。「学びについて教えて」「学びのプロセスについて詳細に掘り下げてみて」のような指示で、これをプロンプトという。
...
//footnote[erukiti-frontiermodel][最先端モデルとは、最低でも1兆円以上のハードウェアと、原子力発電所を専用に建造したくなるほどの莫大な電力消費と、世界のトップの中のトップの研究者・技術者を数百人以上で開発されている、文字通りの最先端モデルである。最先端モデルを開発できる企業は2024年時点ではアメリカにしかない。二番手グループであれば、カナダ、フランス、中国、イギリスなどにも存在している。日本もせめて二番手グループには入りたいものだ。]

無料でChatGPTを使っていると、モデル選択ができずにgpt-4o-miniに該当するモデルしか使えない。要するに無料ユーザーはとても馬鹿なモデルしか使えないのだ。

有料プランには二つの選択肢がある。ChatGPT PlusとChatGPT Proだ。ChatGPT Plusではo1-pro以外が利用できる。この中でo1は強い利用制限がかかっていて、12/6現時点では一週間のリクエスト数は限定されている。つまりお試しでo1が使えるという状況だ。月額＄20（日本円だと3000円くらい）だ。
...
ライバルのClaudeも同様の構造を持っている。Claude 3.5-sonnet（ソネット）は極めて賢い最先端モデルで、特に日本語の扱いでは間違いなくトップクラスの性能を誇る。応答速度はgpt-4oと同程度だ。3.5-haiku（ハイク）はsonnetよりは劣るものの、gpt-4oにも引けを取らない性能を持ち、応答速度はより速い。3-haikuは小さな最先端モデルで、賢さも応答速度もgpt-4o-miniと同程度だ。

Claudeの無料版も3-haikuに該当するモデルが使われている。有料版は2024年12月6日時点では、Professional Planがある。$20で、3.5-sonnetをほぼ使い放題というものだ。

これらのモデルの違いは、単なる性能の差以上の意味を持っている。無料ユーザーは馬鹿なモデルしか使えないため、学びに活用しようと思うと圧倒的に力不足だ。絶対に後悔するので無料モデルなら使わない方がマシだと思った方がいい。ChatGPTが使い物にならないと思っている人の大半は、無料モデルしか使っていない上、プロンプトの基本を知らないので、活用ができていないのだ。

有料プランであるChatGPT PlusやClaude Professional planは、両方とも毎月＄20、日本円で3000円くらいかかるが、その分とても賢い。もちろん、ChatGPT Proの＄200を支払う覚悟があればさらに賢いモデルにアクセスできる@<fn>{erukiti-llm-models}。

//footnote[erukiti-llm-models][無料版は高校生か大学に入ったけど頭の悪い子くらいの性能だが、有料版はトップレベルの大学院生くらいの知的レベルを持っている。特にClaudeの場合はその傾向が顕著だ。ChatGPTでgpt-4oを使う限りそこまで頭は良くないが、大学生から大学院生くらいの知的レベルと言える。o1やo1-proに至ってはその遙か上だ。他のモデルとは異質の賢さ、思慮深さを持っている。]

筆者は生成AIの専門家であるためChatGPT PlusとClaude Professional Planともに契約しているが、普通の人はどちらかを契約するといいだろう。どちらも一長一短だ。軍拡競争としか表現できないような競争を続けている両社@<fn>{erukiti-bigtech-war}なので、性能も実装している機能も、極めて頻繁にアップデートが入ってしまうため、長期的に見てどちらが良いと言えないのだ。
 ...
 * gpt-4o/gpt-4o-mini/o1/o1-mini/o1-proはOpenAI社が開発しているモデル名である
...
実際にClaude ProfessionalのClaude Sonnet@<fn>{erukiti-claude}で問答をしてみよう。対話型AIにおける最も基本的な使い方だ。
//footnote[erukiti-claude][先ほども書いたがClaudeは日本語性能が高いので、筆者は基本的にはClaudeを使っている。]

たとえば「学びにおける自己変容について教えて」と聞いてみよう。

//emlist[Claude Sonnetの返答]{
学びにおける自己変容は、教育や人間の成長において非常に重要なテーマですね。以下の観点から説明させていただきます。
自己変容の本質的な意味は、単なる知識の蓄積ではなく、学びを通じて自分自身の見方や考え方、行動様式が質的に変化することです。
具体的には以下のようなプロセスを経ます：

1. 既存の枠組みへの気づき
まず、自分が持っている前提や思い込み、価値観に気づくことから始まります。例えば、「こういうものだ」と思い込んでいた考えに疑問を持つような経験をします。

2. 認知的不協和の経験
新しい知識や経験が、既存の考え方と衝突することで起こる違和感や混乱を経験します。この不安定な状態は、変容の重要なきっかけとなります。

3. 批判的リフレクション
自分の考え方や行動を振り返り、なぜそのように考えるのか、別の見方はないのかを深く考察します。

4. 新しい意味づけの構築
経験や学びを通じて、物事の見方や解釈の仕方が変化し、新しい理解の枠組みが形成されます。

5. 行動の変容
新しい理解に基づいて、実際の行動様式も変化していきます。

このプロセスにおいて重要なのは：

- 安全な学習環境
変容には不安定さが伴うため、失敗を恐れずに挑戦できる環境が必要です。

- 他者との対話
異なる視点との出会いや、対話を通じた気づきが変容を促進します。

- 実践の機会
新しい理解を実践に移す機会があることで、変容が定着します。

自己変容の意義は、より深い理解と成長を通じて、自分自身と世界との関係性を再構築できることにあります。これは生涯学習の本質的な部分であり、個人の発達において重要な役割を果たします。

このテーマについて、特に興味のある側面はありますか？
//}

このような感じだ。あなたは気が済むまで、対話型AIに、自己変容に関する質問を投げてみればいい。率直な疑問・質問や、感想などを投げてもいい。どんな馬鹿げたと思うような質問でも、モヤモヤしたものでもいい。

なお、この手の問答においてはClaude Sonnetよりもo1/o1-proの方が、より深く賢い答えを返してくれる。

== 活用事例: 文章を書かせる

文章を書かせるということにフォーカスを絞ってみよう。まずは簡単に「学びについて教えて」と投げてみた。
...
たとえば、学びについて本を書きたいと思ったらどうするか？そこで「学びについて本の原稿を書こうと思う。どういう構成が考えられるか？」という指示を投げてみると、AIは序章から終章まで、体系的な構成案を提示してくれる。
...
この構成案は興味深いものだが、より具体的な内容を見てみよう。試しに序章を書かせてみることにした。「試しに序章を書いてみて。2000文字程度で」という指示を出すと、次のような文章が生成された。
...
細かいところは違うものの筆者の文体や主張に近づいた。もっと凝ったプロンプトを与えれば、より精緻になる。

Claude Sonnetは、このように日本語を扱うのが達者なので、原稿を添付して、色々注文を付ければ、それなりにそれっぽい文章を生成してくれる。さてあなたは筆者が書いた原稿のうち、どれくらいが生成AIによるものかわかるだろうか？特に答えは書かないが、実は使っていたのだ。これまで違和感はあっただろうか？もしLLMを使っていたことに気づかなかったなら、それこそがClaude Sonnetの恐るべき力の証明だ。

もっとも、学びのアウトプットとしては、これに頼りすぎると本来の目的を果たせなくなるため注意しなければならない。@<chap>{chap-erukiti2}で見てきたように、学びには実践と振り返りのサイクルが重要だ。AIに文章を書かせるというよりは、構成案を出させるとか、そういう使い方に留めるべきだろう。
...
たとえば、筆者が先日注目していた論文にHtmlRAG@<fn>{erukiti-htmlrag}というものがある。その論文を添付して「解説お願い」と投げてみた。
...
//emlist[Claude Sonnetの返答のうち質問例のみを抜粋]{
質問例：
...
論文などのドキュメントを食わせて問答する目的にはGoogle社のNotebookLMもおすすめだ。これは無料でも問題なく使える。ClaudeやChatGPTにない機能として、Podcast風の音声データを作ってもらう機能がある。
...
この章では、生成AIの現状と、それを学びに活用する方法について見てきた。特に、対話型AIであるChatGPTやClaudeは、文章の生成や理解において優れた性能を持っており、学びのプロセスを強力に支援してくれる存在だ。

筆者が強調したいのは、対話型AIを活用する際の二つの重要な点だ。一つは、無料版と有料版で性能に大きな差があるということだ。無料版は極めて限定的な性能しか持っていないため、本格的な学びの支援には有料版の利用を推奨する。もう一つは、生成AIの出力を無条件に信用してはいけないということだ。特にマイナーな分野や最新の情報については、必ず検証が必要になる。

対話型AIは、文章の作成支援、論文の理解、学習課題の生成など、様々な場面で活用できる。ただし、これはあくまでも支援ツールであり、学びの本質である「実践と振り返りのサイクル」や「言語化」の重要性は変わらない。対話型AIを上手く活用することで、より効率的で深い学びが可能になるのだ。

技術の進歩は目覚ましく、あなたがこれを読んでいる頃には、さらに新しい機能や可能性が生まれているかもしれない。重要なのは、これらのツールを自分の学びにどう活かすかという視点を持つことだ。生成AIは、我々の知的活動を支援する強力なツールではあるが、それを使いこなすのは結局のところ人間自身なのだ。
